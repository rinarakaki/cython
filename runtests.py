...
def update_cpp_extension(
    cpp_std, min_gcc_version=None, min_clang_version=None, min_macos_version=None
):
    def _update_cpp_extension(ext):
        ...
        if gcc_version:
            ...
            if not already_has_std:
                ...
                if not min_gcc_version or float(compiler_version) >= float(
                    min_gcc_version
                ):
                    use_gcc = True
        ...
        if clang_version:
            ...
            if not already_has_std:
                ...
                if not min_clang_version or float(compiler_version) >= float(
                    min_clang_version
                ):
                    use_clang = True
                    ext.extra_compile_args.append("-std=c++%s" % cpp_std)
...
def get_cc_version(language):
    ...
    try:
        ...
        warnings.warn(
            "Unable to find the %s compiler: %s: %s"
...
EXT_EXTRAS = {
    ...
    "tag:cpp11": update_cpp_extension(
        11, min_gcc_version="4.9", min_macos_version="10.7"
    ),
    "tag:cpp17": update_cpp_extension(
        17, min_gcc_version="5.0", min_macos_version="10.13"
    ),
    "tag:cpp20": update_cpp_extension(
        20, min_gcc_version="11.0", min_clang_version="13.0", min_macos_version="10.13"
    ),
        ...
        for line in f:
            ...
            if parsed is not None:
                ...
                if tag not in (
                    ...
                    if tag == "tags":
                        raise RuntimeError(
                            "test tags use the 'tag' directive, not 'tags' (%s)"
                            % filepath
                        )
                    print(
                        "WARNING: unknown test directive '%s' found (%s)"
                        % (tag, filepath)
                    )
                values = values.split(",")
...
list_unchanging_dir = memoize(
    lambda x: os.listdir(x)
)  # needs lambda to set function attribute
...
def _list_pyregr_data_files(test_directory):
    ...
    return ["__init__.py"] + [
        filename
        for filename in list_unchanging_dir(test_directory)
...
class ErrorWriter(object):
    match_error = re.compile(
        r"(?:(warning|performance hint):)?(?:.*:)?\s*([-0-9]+)\s*:\s*([-0-9]+)\s*:\s*(.*)"
    ).match
    ...
    def _collect(self):
        ...
        for line in s.splitlines():
            ...
            if match:
                ...
                results[message_type or "error"].append(
                    (int(line), int(column), message.strip())
                )
...
class Stats(object):
    ...
    def print_stats(self, out=sys.stderr):
        if not self.test_times:
            return
        lines = ["Times:\n"]
        for metric, t in sorted(
            self.test_times.items(), key=operator.itemgetter(1), reverse=True
        ):
            count = self.test_counts[metric]
            top = self.top_tests[metric]
            lines.append(
                "%-12s: %8.2f sec  (%4d, %6.3f / run) - slowest: %s\n"
                % (
                    metric,
                    t,
                    count,
                    t / count,
                    ", ".join(
                        "'{2}:{1}' ({0:.2f}s)".format(*item)
                        for item in heapq.nlargest(self.top_n, top)
                    ),
                )
            )
...
class TestBuilder(object):
    def __init__(
        ...
        self.test_selector = (
            re.compile(options.only_pattern).search if options.only_pattern else None
        )
        ...
        for filename in filenames:
            ...
            if os.path.isdir(path) and filename != TEST_SUPPORT_DIR:
                ...
                if filename == "broken" and not self.test_bugs:
                    continue
                suite.addTest(self.handle_directory(path, filename))
        if (
            ...
            if [1 for selector in self.selectors if selector("embedded")] and not [
    ...
    def handle_directory(self, path, context):
        ...
        for filename in filenames:
            ...
            if not [1 for match in self.selectors if match(fqmodule, tags)]:
                continue
            if self.exclude_selectors:
                if [1 for match in self.exclude_selectors if match(fqmodule, tags)]:
                    continue
            ...
            if (
                ...
                min_py_ver = [
                    ...
                    for pyver in map(
                        re.compile(r"pure([0-9]+)[.]([0-9]+)").match, tags["tag"]
                    )
                    if pyver
                ]
                if not min_py_ver or any(
                    sys.version_info >= min_ver for min_ver in min_py_ver
                ):
                    suite.addTest(
                        PureDoctestTestCase(
                            module,
    ...
    def build_tests(
        ...
        if (
            ...
            and "cpp" in tags["tag"]
            and not "no-cpp-locals" in tags["tag"]
        ):
    ...
    def build_test(
        ...
        if extra_directives:
            workdir += "_directives_" + "_".join(
                "%s_%s" % (k, v) for k, v in extra_directives.items()
            )
        return test_class(
            path,
            workdir,
...
class CythonCompileTestCase(unittest.TestCase):
        ...
        if self.add_cython_import:
            with open(self.module_path, "rb") as f:
                ...
                if b"cython.cimports." in source:
                    ...
                    for name in set(
                        re.findall(rb"(cython\.cimports(?:\.\w+)+)", source)
                    ):
                        name = name.decode()
    ...
    def split_source_and_output(self, source_file, workdir, add_cython_import=False):
        ...
        with io_open(source_file, "r", encoding="ISO-8859-1") as source_and_output:
            ...
            out = io_open(
                os.path.join(workdir, os.path.basename(source_file)),
                "w",
                encoding="ISO-8859-1",
            )
            try:
        ...
        return (
            error_writer.geterrors() if error_writer else [],
    ...
    def run_cython(
        ...
        include_dirs = INCLUDE_DIRS + [
            os.path.join(test_directory, "..", TEST_SUPPORT_DIR)
        ]
        if incdir:
    ...
    def run_distutils(
        self, test_directory, module, workdir, incdir, extra_extension_args=None
    ):
        cwd = os.getcwd()
        os.chdir(workdir)
        try:
    ...
    def compile(
        ...
        expected_errors = expected_warnings = expected_perf_hints = errors = (
            warnings
        ) = perf_hints = ()
        ...
        if expect_errors or expect_warnings or expect_perf_hints or add_cython_import:
            expected_errors, expected_warnings, expected_perf_hints = (
                self.split_source_and_output(module_path, workdir, add_cython_import)
            )
            ...
            if self.test_determinism and not expect_errors:
                ...
                for file in os.listdir(workdir2):
                    ...
                    if txt1 != txt2:
                        diffs.append(file)
                        os.system(
                            "diff -u %s/%s %s/%s > %s/%s.diff"
                            % (workdir, file, workdir2, file, workdir2, file)
                        )
                if diffs:
                ...
                with captured_fd(1) as get_stdout:
                    with captured_fd(2) as get_stderr:
                        with self.stats.time(
                            self.name, self.language, "compile-%s" % self.language
                        ):
                            so_path = self.run_distutils(
                                test_directory, module, workdir, incdir
                            )
            except Exception as exc:
                if "cerror" in self.tags["tag"] and (
                    (get_stderr and get_stderr()) or isinstance(exc, CCompilerError)
                ):
...
class PureDoctestTestCase(unittest.TestCase):
    ...
    def shortDescription(self):
        return "[%d] running pure doctests in %s" % (self.shard_num, self.module_name)
...
class PartialTestResult(TextTestResult):
    def __init__(self, base_result):
        TextTestResult.__init__(
            self, self._StringIO(), True, base_result.dots + base_result.showAll * 2
        )
        ...
        for test_case, error in results:
            for attr_name in filter(is_private_field, dir(test_case)):
                if attr_name == "_dt_test":
                    test_case._dt_test = _FakeClass(name=test_case._dt_test.name)
...
class CythonPyregrTestCase(CythonRunTestCase):
    def setUp(self):
        ...
        Options._directive_defaults.update(
            dict(
                binding=True, always_allow_keywords=True, set_initial_path="SOURCEFILE"
            )
        )
        ...
        for cls in classes:
            if isinstance(cls, str):
                if cls in sys.modules:
                    suite.addTest(
                        unittest.TestLoader().loadTestsFromModule(sys.modules[cls])
                    )
                else:
...
class TestCodeFormat(unittest.TestCase):
    ...
    def runTest(self):
        ...
        for codedir in source_dirs:
            paths += glob.glob(
                os.path.join(self.cython_dir, codedir + "/**/*.py"), recursive=True
            )
        ...
        for codedir in ["Cython", "Demos", "pyximport"]:  # source_dirs:
            paths += glob.glob(
                os.path.join(self.cython_dir, codedir + "/**/*.p[yx][xdi]"),
                recursive=True,
            )
        ...
        if package_matches(parentname):
            for f in filenames:
                if file_matches(f):
                    ...
                    modulename = module_prefix + filepath[len(path) + 1 :].replace(
                        os.path.sep, "."
                    )
...
def collect_doctests(path, module_prefix, suite, selectors, exclude_selectors):
    ...
    def file_matches(filename):
        ...
        return (
            ...
            and not filename in excludelist
        )
...
class EndToEndTest(unittest.TestCase):
    ...
    def shortDescription(self):
        return "[%d] End-to-end %s" % (self.shard_num, self.name)
    ...
    def runTest(self):
        ...
        for command_no, command in enumerate(self.commands, 1):
            ...
            time_category = (
                ...
                if (
                    "setup.py" in command
                    or "cythonize.py" in command
                    or "cython.py" in command
                )
                else "etoe-run"
            )
            with self.stats.time(
                "%s(%d)" % (self.name, command_no), "c", time_category
            ):
                if self.capture:
                    p = subprocess.Popen(
                        command,
            ...
            if res != 0:
                ...
                sys.stderr.write(
                    "Final directory layout of '%s':\n%s\n\n"
                    % (
                        self.name,
                        "\n".join(
                            os.path.join(dirpath, filename)
                            for dirpath, dirs, files in os.walk(self.workdir)
                ...
                self.assertEqual(
                    0,
                    res,
                    "non-zero exit status, last output was:\n%r\n-- stdout:%s\n-- stderr:%s\n"
...
class EmbedTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.system("make PYTHON='%s' clean > /dev/null" % sys.executable)
    ...
    def test_embed(self):
        ...
        if not os.path.isdir(libdir) or libname not in os.listdir(libdir):
            ...
            if not os.path.isdir(libdir) or libname not in os.listdir(libdir):
                libdir = os.path.join(
                    libdir, "python%d.%d" % sys.version_info[:2], "config"
                )
                if not os.path.isdir(libdir) or libname not in os.listdir(libdir):
...
class MissingDependencyExcluder(object):
    ...
    def find_dep_version(self, name, module):
        try:
            ...
            module_path = getattr(
                module, "__file__", stdlib_dir
            )  # no __file__? => builtin stdlib module
...
class FileListExcluder(object):
    ...
    def __call__(self, testname, tags=None):
        ...
        if exclude and self.verbose:
            print(
                "Excluding %s because it's listed in %s" % (testname, self._list_file)
            )
        return exclude
...
class ShardExcludeSelector(object):
    ...
    def __call__(self, testname, tags=None, _hash=zlib.crc32):
        ...
        return (
            _hash(self._seed + testname.encode()) % self.shard_count != self.shard_num
        )
...
def get_version():
    ...
    if os.path.exists(os.path.join(top, ".git")):
        ...
        try:
            ...
            version_commit = subprocess_output(
                ["git", "rev-parse", cython_version]
            ).strip()
...
def main():
    ...
    parser.add_option(
        "--compiler", dest="compiler", default=None, help="C compiler type"
    )
    ...
    parser.add_option(
        "--root-dir",
        dest="root_dir",
        default=os.path.join(DISTDIR, "tests"),
        help=(
            "Directory to look for the file based "
    ...
    parser.add_option(
        "--work-dir",
        dest="work_dir",
        default=os.path.join(os.getcwd(), "TEST_TMP"),
    ...
    if options.shard_count > 1 and options.shard_num == -1:
        ...
        tasks = [
            (options, cmd_args, shard_num) for shard_num in range(options.shard_count)
        ]
        ...
        with time_stamper_thread(interval=keep_alive_interval, open_shards=open_shards):
            for (
                ...
                return_code,
                failure_output,
            ) in pool.imap_unordered(runtests_callback, tasks):
                ...
                for stage_name, (stage_time, stage_count) in pipeline_stats.items():
                    ...
                    merged_pipeline_stats[stage_name] = (
                        old_time + stage_time,
                        old_count + stage_count,
                    )
        ...
        sys.stderr.write(
            "Sharded tests run in %d seconds (%.1f minutes)\n"
            % (round(total_time), total_time / 60.0)
        )
        if error_shards:
            sys.stderr.write(
                "Errors found in shards %s\n"
        ...
        with time_stamper_thread(interval=keep_alive_interval):
            _, stats, merged_pipeline_stats, return_code, _ = runtests(
                options, cmd_args, coverage
            )
    ...
    pipeline_stats = [
        (
            as_msecs(stage_time),
            as_msecs(stage_time) / stage_count,
            stage_count,
            stage_name,
        )
        for stage_name, (stage_time, stage_count) in merged_pipeline_stats.items()
    ]
    total_pipeline_time_percent = (
        math.fsum(stats[0] for stats in pipeline_stats) / 100.0
    )
    pipeline_stats.sort(reverse=True)
    sys.stderr.write(
        "Most expensive pipeline stages: %s\n"
        % ", ".join(
            "%r: %.2f / %d (%.3f / run, %.1f%%)"
            % (
                stage_name,
                ...
                stage_count,
                stage_time,
                total_stage_time / total_pipeline_time_percent,
            )
            for total_stage_time, stage_time, stage_count, stage_name in pipeline_stats[
                :10
            ]
        )
    )
...
def configure_cython(options):
    ...
    DebugFlags.debug_no_exception_intercept = (
        1  # provide better crash output in CI runs
    )
...
def runtests_callback(args):
    ...
    try:
        runtests.__code__ = runtests.__code__.replace(
            co_name=f"runtests_SHARD_{shard_num}"
        )
...
def runtests(options, cmd_args, coverage=None):
    ...
    if options.shard_num > -1:
        ...
        if xml_output_dir:
            xml_output_dir = os.path.join(
                xml_output_dir, "shard-%03d" % options.shard_num
            )
    ...
    if options.for_debugging:
        ...
        if WITH_CYTHON and include_debugger:
            from Cython.Compiler.Options import (
                default_options as compiler_default_options,
            )
    ...
    try:
        ...
        if not (
            [0, 9] <= list(map(int, re.findall("[0-9]+", jedi.__version__ or "0")))
        ):
            raise ImportError
    except (ImportError, AttributeError, TypeError):
    ...
    if options.shard_num > -1:
        exclude_selectors.append(
            ShardExcludeSelector(options.shard_num, options.shard_count)
        )
    ...
    if not test_bugs:
        ...
        exclude_selectors += [
            FileListExcluder(
                os.path.join(ROOTDIR, bugs_file_name), verbose=verbose_excludes
            )
            for bugs_file_name, condition in bug_files
            if condition
        ]
    ...
    exclude_selectors += [
        TagsSelector("tag", tag) for tag, exclude in TAG_EXCLUDERS if exclude
    ]
    ...
    selected_backends = [
        name.strip() for name in options.backends.split(",") if name.strip()
    ]
    backends = []
    ...
    if options.examples and languages:
        ...
        for subdirectory in glob.glob(os.path.join(options.examples_dir, "*/")):
            filetests = TestBuilder(
                ...
                stats=stats,
                add_cython_import=True,
            )
            test_suite.addTest(filetests.build_suite())

    if options.system_pyregr and languages:
        sys_pyregr_dir = os.path.join(
            sys.prefix, "lib", "python" + sys.version[:3], "test"
        )
        if not os.path.isdir(sys_pyregr_dir):
            sys_pyregr_dir = os.path.join(
                os.path.dirname(sys.executable), "Lib", "test"
            )  # source build
    ...
    if xml_output_dir:
        ...
        test_runner = XMLTestRunner(
            output=xml_output_dir, verbose=options.verbosity > 0
        )
        if options.failfast:
        ...
        test_runner = unittest.TextTestRunner(
            verbosity=options.verbosity, **text_runner_options
        )
    ...
    if options.pyximport_py:
        ...
        pyximport.install(
            pyimport=True,
            build_dir=os.path.join(WORKDIR, "_pyximport"),
    ...
    try:
        if options.shard_num > -1:
            thread_id = (
                f" (Thread ID 0x{threading.get_ident():x})"
                if threading is not None
                else ""
            )
            sys.stderr.write(
                f"Tests in shard ({options.shard_num}/{options.shard_count}) starting{thread_id}\n"
            )
        ...
        if options.shard_num > -1:
            sys.stderr.write(
                f"Tests in shard ({options.shard_num}/{options.shard_count}) crashed: {exc}\n"
            )
        import traceback
