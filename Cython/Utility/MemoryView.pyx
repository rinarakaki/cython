...
cdef class memoryview:
    ...
    fn setitem_slice_assign_scalar(self, memoryview dst, value):
        ...
        try:
            if self.dtype_is_object:
                ...
                self.assign_item_from_object(<&mut char>item, value)
    ...
    fn setitem_indexed(self, index, value):
        let &mut char itemp = self.get_item_pointer(index)
    ...
    fn assign_item_from_object(self, &mut char itemp, object value):
        """Only used if instantiated manually by the user, or if Cython doesn't
        know how to convert the type"""
        import r#struct
        let char c
...
fn bytes format_from_typeinfo(__Pyx_TypeInfo *type):
    let &mut __Pyx_StructField field
