...
extern from "<unistd.h>" nogil:
    ...
    int          pause()
    int          pipe(int [2])
    ssize_t      pread(int, void *, usize, off_t)
    ssize_t      pwrite(int, const void *, usize, off_t)
    ssize_t      read(int, void *, usize)
    ...
    char        *ttyname(i32)
    int          ttyname_r(int, char *, usize)
    useconds_t   ualarm(useconds_t, useconds_t)
    int          unlink(const char *)
    int          usleep(useconds_t)
    pid_t        vfork()
    ssize_t      write(int, const void *, usize)
    char         *optarg
    int          optind
    int          opterr
    int          optopt
