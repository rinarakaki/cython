...
extern from "<stdlib.h>" nogil:
    ...
    fn i32 atoi(&char string)
    fn i64 atol(&char string)
    fn i128 atoll(&char string)
    fn f64 atof(&char string)
    fn i64 strtol(&char string, &char* tailptr, i32 base)
    fn u64 strtoul(&char string, &char* tailptr, i32 base)
    fn i128 strtoll(&char string, &char* tailptr, i32 base)
    fn u128 strtoull(&char string, &char* tailptr, i32 base)
    fn f32 strtof(&char string, &char* tailptr)
    fn f64 strtod(&char string, &char* tailptr)
    fn f128 strtold(&char string, &char* tailptr)
