...
def load_dynamic(name, path):
    spec = importlib.util.spec_from_file_location(
        name, loader=ExtensionFileLoader(name, path)
    )
...
def cython_inline(
    ...
    **kwds,
    ...
    ctx = (
        ...
        if cython_include_dirs
        else _cython_inline_default_context
    )

    cython_compiler_directives = (
        dict(cython_compiler_directives) if cython_compiler_directives else {}
    )
...
class RuntimeCompiledFunction:
    ...
    def __call__(self, *args, **kwds):
        ...
        return cython_inline(
            self._body, locals=self._f.__globals__, globals=self._f.__globals__, **all
        )
