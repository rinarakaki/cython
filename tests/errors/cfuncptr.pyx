...
fn int exceptmaybeminus2(int bad) except ?-2:
    if bad:
        raise RuntimeError
    else:
        return 0
...
def fail_exceptmaybeminus2(bad):
    let int (*fptr_a)(int) except -2
    let int (*fptr_b)(int) except -1
    let int (*fptr_c)(int) except ?-1
...
extern from *:
    ...
    let int exceptstar(int bad) except *
