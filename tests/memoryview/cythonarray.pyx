...
def acquire():
    ...
    >>> acquire()
    '''
    let object[int, ndim=1, mode="c"] buf1d = \
            array(shape=(10,), itemsize=sizeof(int), format='i', mode='c')
    let object[int, ndim=2, mode="c"] buf2d = \
            array(shape=(10,10), itemsize=sizeof(int), format='i')
    let object[unsigned long, ndim=3, mode='fortran'] buf3d = \
...
def full_or_strided():
    ...
    >>> full_or_strided()
    '''
    let object[float, ndim=2, mode='full'] fullbuf = \
            array(shape=(10,10), itemsize=sizeof(float), format='f', mode='c')
    let object[long long int, ndim=3, mode='strided'] stridedbuf = \
...
def test_cython_array_getbuffer():
    ...
    61
    """
    fn int[:, ::1] cslice = create_array((14, 10), 'c')
    fn int[::1, :] fslice = create_array((14, 10), 'fortran')
...
fn int *getp(int dim1=10, int dim2=10, dim3=1) except NULL:
    print "getp()"

    let int *p = <int *> malloc(dim1 * dim2 * dim3 * sizeof(int))
