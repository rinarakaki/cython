...
def test_prange():
    ...
    (9, 9, 45, 45)
    """
    let Py_ssize_t i, j, sum1 = 0, sum2 = 0
...
def test_descending_prange():
    ...
    5
    """
    let int i, start = 5, stop = -5, step = -2
    let int sum = 0
...
def test_prange_matches_range(int start, int stop, int step):
    ...
    """
    let int i = -765432, range_last = -876543, prange_last = -987654
    prange_set = set()
...
def test_propagation():
    ...
    (9, 9, 9, 9, 450, 450)
    """
    let int i = 0, j = 0, x = 0, y = 0
    let int sum1 = 0, sum2 = 0
...
#    let int i
#    let int start = -5
#    let unsigned int stop = 5
#    let int step = 1
#
#    let int steps_taken = 0
#    let int *steps_takenp = &steps_taken
#
#    for i in prange(start, stop, step, nogil=True):
#        steps_taken += 1
...
def test_reassign_start_stop_step():
    ...
    20
    """
    let int start = 0, stop = 10, step = 2
    let int i
    let int sum = 0
...
def test_nan_init():
    ...
    let unsigned char a2 = 10
    let short b1 = 10
    let unsigned short b2 = 10
    let int c1 = 10
    let unsigned int c2 = 10
    let long d1 = 10
    let unsigned long d2 = 10
    let long long e1 = 10
    let unsigned long long e2 = 10
...
def test_prange_continue():
    ...
    """
    let int i
    let int *p = <int *> calloc(10, sizeof(int))
...
def test_nested_break_continue():
    ...
    8
    """
    let int i, j, result1 = 0, result2 = 0
...
fn int parallel_return() noexcept nogil:
    let int i
...
fn int parallel_exc_cdef() except -3:
    let int i, j
    for i in prange(10, nogil=True):
...
fn int parallel_exc_cdef_unnested() except -3:
    let int i
    for i in prange(10, nogil=True):
        with gil:
            raise Exception("propagate me")
...
fn int parallel_exc_nogil_swallow() except -1:
    let int i, j
    for i in prange(10, nogil=True):
        try:
            for j in prange(10):
...
fn int parallel_exc_nogil_swallow_unnested() except -1:
    let int i
    with nogil:
        try:
            for i in prange(10):
...
fn int inner_parallel_section() noexcept nogil:
    let int j, sum = 0
    for j in prange(10):
        sum += j
    return sum
...
fn int nogil_cdef_except_clause() except -1 nogil:
    return 1
...
fn int chunksize() noexcept nogil:
    return 3
...
def test_prange_call_exception_checked_function():
    ...
    let int N = 10000
    let int* buf = <int*>malloc(sizeof(int)*N)
    if buf == NULL:
        raise MemoryError
    try:
