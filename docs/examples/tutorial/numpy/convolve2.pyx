...
def naive_convolve(cnp.ndarray f, cnp.ndarray g):
    ...
    # array indices.
    let int vmax = f.shape[0]
    let int wmax = f.shape[1]
    let int smax = g.shape[0]
    let int tmax = g.shape[1]
    let int smid = smax // 2
    let int tmid = tmax // 2
    let int xmax = vmax + 2 * smid
    let int ymax = wmax + 2 * tmid
    ...
    # Python objects).
    let int s_from, s_to, t_from, t_to
