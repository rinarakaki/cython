...
pxd_include_dirs = [
    directory
    for directory, dirs, files in os.walk(os.path.join("Cython", "Includes"))
    if "__init__.pyx" in files
    or "__init__.pxd" in files
...
setup_args["package_data"] = {
    ...
    + ["py.typed", "__init__.pyi", "Shadow.pyi"],
...
def compile_cython_modules(
    ...
    pgen = find_executable(
        "pgen",
        os.pathsep.join(
            [os.environ["PATH"], os.path.join(get_python_inc(), "..", "Parser")]
        ),
    )
    if not pgen:
...
def run_build():
    if compile_cython_itself and (
        is_cpython or cython_compile_more or cython_compile_minimal
    ):
        compile_cython_modules(
            cython_profile,
            cython_coverage,
            cython_compile_minimal,
            cython_compile_more,
